...
static void Ph_Shift_Comm(void);
...
volatile extern INT16U HV_Cut, R_HV_Cut, Y_HV_Cut, B_HV_Cut;
volatile extern INT8U LV_Cut, R_LV_Cut, Y_LV_Cut, B_LV_Cut;
volatile extern INT16U VA_Max, R_VA_Max, Y_VA_Max, B_VA_Max;

volatile extern BOOL PH3;													//	Variable to represent 1PH(0) or 3PH(1) SYSTEM
...
/********************************************************************
*																	*
*	TO SIMULATE 3 PHASE USING 1 PHASE MULTIPLEXER BOARD CONNECTED	*
*																	*
********************************************************************
...
static void Tx_Dimm_Wt_All_Cmds(void)
{
	...
	if(Dimm_W_All_Cmd_Indx >= MAX_WT_ALL_CMD_INDX)         
	{
		...
		Dimm_W_All_Cmd_Indx = 0;
			...
			Debug_PutString("\n\r W_ALL_CMD");
		#endif
		Ph_Shift_Comm();														//	Shift Communication w.r.t Phase
	}
	if(En_Wt_All_Cmd ==	TRUE )
	{
		Dimm_Comm_PutString(Dimm_W_All_Cmds[Dimm_W_All_Cmd_Indx]);
		if(Dimm_W_All_Cmd_Indx == 1)
		{
			...
			Data2Dim = Smooth_Dim(Dim_OPV, Data2Dim);	
			
			#ifdef	P_Debug
				Debug_PutString("\n\r Dim_OPV:");
				Debug_PutInt(Dim_OPV);
				Debug_PutString("\n\r DATA2DIM:");
				Debug_PutInt(Data2Dim);
				Debug_PutString("\n\r");
			#endif
...
**																											**
** 	Arguments  		: 	void																				**			
**						Argument 1:	NO					Scope/Range :	-									**
** 						Argument 2:	NO					Scope/Range :	-									**
** 	Returns    		: 	void																				**	
**																											**
** 	Pointers		:	NO								Tremination:	-									**
**	G Variables		:																						**
**						Defined in:																			**			
**																											**					
...
static void Tx_Dimm_Wt_OpV_Cmds(void)
{
	...
	{
		Dimm_Comm_PutString(Dimm_W_OpV_Cmd[Dimm_W_OpV_Cmd_Indx]);
...
static void Tx_Dimm_Wt_Cmds(void)
{
	...
	if(En_Wt_Cmd == TRUE)
	{
		Dimm_Comm_PutString(Dimm_W_Cmd[Dimm_W_Cmd_Indx]);
...
static void Cmp_Rd_Cmds_Data(void)
{
	...
	if(PH3)
	{
		if( (R_D_OPV != R_D_M_ExpV) || (Y_D_OPV != Y_D_M_ExpV) || (B_D_OPV != B_D_M_ExpV) )
		{
			En_Wt_OpV_Cmd = TRUE;
		}	
		else
		...
		if( (R_D_VA != R_VA_Max) || ( R_D_HV_Cut != R_HV_Cut) || ( R_D_LV_Cut != R_LV_Cut) || \
			(Y_D_VA != Y_VA_Max) || ( Y_D_HV_Cut != Y_HV_Cut) || ( Y_D_LV_Cut != Y_LV_Cut) || \
			(B_D_VA != B_VA_Max) || ( B_D_HV_Cut != B_HV_Cut) || ( B_D_LV_Cut != B_LV_Cut) )
		{
			En_Wt_Cmd = TRUE;
		}		    	   
		else
		...
		if( (En_Wt_OpV_Cmd == TRUE) && (En_Wt_Cmd == TRUE) )
			En_Wt_All_Cmd	=	TRUE;
		else if( (En_Wt_OpV_Cmd == FALSE) && (En_Wt_Cmd == FALSE) )
		{
			#ifdef	P_Debug
				Debug_PutString("\n\r No Write");
			#endif
	...
	{	
		...
		if( (R_D_VA != R_VA_Max) || ( R_D_HV_Cut != R_D_M_HV_Cut) || ( R_D_LV_Cut != R_D_M_LV_Cut))		
		{
			En_Wt_Cmd = TRUE;
		}		    	   
		else
...
static void Capture_Rx_Data(void)
{
	...
	/*************************************************************
	Write No responce logic here Phase sequence shift
	*************************************************************/
...
void Parse_Dimmer_Data(INT16S Cmd_Indx, INT16S len)
{
	...
	switch(Cmd_Indx)
	{
		...
		case D_TEMP:															//	Read Temperature from Dimmer
			...
			if(EN_PHASE_R)
				R_D_Temp = (INT16U)Rx_Dimm_Data;								//	Phase-R Temperature
			else if(EN_PHASE_Y)
				Y_D_Temp = (INT16U)Rx_Dimm_Data;								//	Phase-Y Temperature	
			else if(EN_PHASE_B)
				B_D_Temp = (INT16U)Rx_Dimm_Data;								//	Phase-B Temperature
			break;
		case D_IN_V:															//	Read Input Voltage from Dimmer
			...
			if(EN_PHASE_R)
				R_D_IPV = (INT16U)Rx_Dimm_Data;									//	Phase-R Input Voltage
			else if(EN_PHASE_Y)
				Y_D_IPV = (INT16U)Rx_Dimm_Data;									//	Phase-Y Input Voltage
			else if(EN_PHASE_B)
				B_D_IPV = (INT16U)Rx_Dimm_Data;									//	Phase-B Input Voltage
			break;
		case D_OP_V:															//	Read Output Voltage from Dimmer			
			...
			Rx_Dimm_Data = Read_Dimm_Resp(RecievedData, len)/100;
			if(EN_PHASE_R)
				R_D_OPV = (INT16U)Rx_Dimm_Data;									//	Phase-R Output Voltage	
			else if(EN_PHASE_Y)
				Y_D_OPV = (INT16U)Rx_Dimm_Data;									//	Phase-Y Output Voltage	
			else if(EN_PHASE_B)
				B_D_OPV = (INT16U)Rx_Dimm_Data;									//	Phase-B Output Voltage	
		    break;
		case D_LOAD:															//	Read Load in VA from Dimmer
			...
			Rx_Dimm_Data = Read_Dimm_Resp(RecievedData, len);
			if(EN_PHASE_R)
				R_D_VA = (INT16U)Rx_Dimm_Data;									//	Phase-R VA	
			else if(EN_PHASE_Y)
				Y_D_VA = (INT16U)Rx_Dimm_Data;									//	Phase-Y VA
			else if(EN_PHASE_B)
				B_D_VA = (INT16U)Rx_Dimm_Data;									//	Phase-B VA
			break;
		case D_FREQ:															//	Read Frequency from Dimmer
			...
			if(EN_PHASE_R)
				R_D_Freq = (INT16U)Rx_Dimm_Data;								//	Phase-R Frequency			
			else if(EN_PHASE_Y)
				Y_D_Freq = (INT16U)Rx_Dimm_Data;								//	Phase-Y Frequency	
			else if(EN_PHASE_B)
				B_D_Freq = (INT16U)Rx_Dimm_Data;								//	Phase-B Frequency
			break;
		case D_FLAGS:															//	Read 16 Bit flag status from Dimmer	
			 ...
			 Rx_Dimm_Data = Read_Dimm_Resp(RecievedData, len);
			if(EN_PHASE_R)
				R_D_Flags = (INT16U)Rx_Dimm_Data;								//	Phase-R 16 bit Flags	
			else if(EN_PHASE_Y)
				Y_D_Flags = (INT16U)Rx_Dimm_Data;								//	Phase-Y 16 bit Flags	
			else if(EN_PHASE_B)
				B_D_Flags = (INT16U)Rx_Dimm_Data;								//	Phase-B 16 bit Flags
		...
		case D_S_OP_V:															//	Read Output Voltage Set value from Dimmer
			...
			Rx_Dimm_Data = Read_Dimm_Resp(RecievedData, len)/100;
			if(EN_PHASE_R)
				R_D_ExpV = (INT16U)Rx_Dimm_Data;								//	Phase-R Dimmer Output Set Voltage Value
			else if(EN_PHASE_Y)
				Y_D_ExpV = (INT16U)Rx_Dimm_Data;								//	Phase-Y Dimmer Output Set Voltage Value
			else if(EN_PHASE_B)
				B_D_ExpV = (INT16U)Rx_Dimm_Data;								//	Phase-B Dimmer Output Set Voltage Value
			break;
		case D_S_MAX_LOAD:														//	Read Max_load in VA set value from Dimmer
			...
			Rx_Dimm_Data = Read_Dimm_Resp(RecievedData, len);					
			if(EN_PHASE_R)
				R_D_Max_VA = (INT16U)Rx_Dimm_Data;
			else if(EN_PHASE_Y)
				Y_D_Max_VA = (INT16U)Rx_Dimm_Data;
			else if(EN_PHASE_B)
				B_D_Max_VA = (INT16U)Rx_Dimm_Data;
			break;
		case D_S_HV_CUT:														//	Read HV_Cut set value from Dimmer
			 ...
			 Rx_Dimm_Data = Read_Dimm_Resp(RecievedData, len)/100;
			if(EN_PHASE_R)
				R_D_HV_Cut = (INT16U)Rx_Dimm_Data;
			else if(EN_PHASE_Y)
				Y_D_HV_Cut = (INT16U)Rx_Dimm_Data;
			else if(EN_PHASE_B)
				B_D_HV_Cut = (INT16U)Rx_Dimm_Data;
			break;
		case D_S_LV_CUT:
			#ifdef	P_Debug
			...
			Rx_Dimm_Data = Read_Dimm_Resp(RecievedData, len)/100;
			if(EN_PHASE_R)
				R_D_LV_Cut = (INT16U)Rx_Dimm_Data;
			else if(EN_PHASE_Y)
				Y_D_LV_Cut = (INT16U)Rx_Dimm_Data;
			else if(EN_PHASE_B)
				B_D_LV_Cut = (INT16U)Rx_Dimm_Data;
			break;
...
INT32U Read_Dimm_Resp(INT8U * DataPtr, INT16 len)
{
	INT8U i = 0;
	INT8U data = 0;
	volatile INT32U DimmValue = 0;
	while(len > 0 )
	{
		 data = DataPtr[i];
		#ifdef	Debug_Dim_Resp
			 Debug_PutChar(data);										// To display Received RAW Data from dimmer
		#endif
		 if((data >= 48) && (data <= 57))								// Strings to integer conversion Capture Numeric Values Only
		 {	
...
static INT16U Smooth_Dim(INT16U Rd_OPV, INT16U Exp_OPV)											//	Regulate Output Voltage Smoothly
{
	...
	if( ( (Rd_OPV - Exp_OPV) < Adj_Volts) || ( (Exp_OPV - Rd_OPV ) < Adj_Volts))
		S_Dimm_Value  = Exp_OPV;
	else if( Rd_OPV > Exp_OPV )																		//	Decrease Ouput Voltage if Existing Output is greater than expected		
	{
...
static INT16U RYB_VA(void)
{
	volatile INT16U Temp_VA;
	if(EN_PHASE_R)
	{	
		if(R_VA_Max < R_D_M_Max_VA)																//	Phase _R VA MAX Range
			Temp_VA = R_VA_Max;
		else
			Temp_VA = Default_Max_VA;
	}   	
	else if(EN_PHASE_Y)																			//	Phase _Y VA MAX Range	
	{
	...
	else if(EN_PHASE_B)																			//	Phase _B VA MAX Range
	{
...
static void Ph_Shift_Comm(void)
{
	if(PH3)
	{
		...
		else if(EN_PHASE_B)
		{
			...
			EN_PHASE_Y = 0;
			EN_PHASE_B = 0;  
			Dimm_Comm_En_Flag = FALSE;					//	Communication with 3 Phase's Completed, Disable Dimmer Communication.		
		...
		Dimm_Comm_En_Flag = FALSE;					//	Communication with Single Phase Completed, Disable Dimmer Communication.		
}
void Debug_Dimmer(void)
{
	...
	{
		Debug_PutString("\n\rY_D_ExpV \t:");
		Debug_PutInt(Y_D_ExpV);	
		
		
		Debug_PutString("    Y_D_M_ExpV \t:");
		Debug_PutInt(Y_D_M_ExpV);
		
		Debug_PutString("\n\rY_D_Max_VA \t:");
		Debug_PutInt(Y_D_Max_VA);	
		Debug_PutString("    Y_VA_Max \t:");
		Debug_PutInt(Y_VA_Max);
		
		Debug_PutString("\n\rY_D_HV_Cut \t:");
		Debug_PutInt(Y_D_HV_Cut);	
		Debug_PutString("    Y_D_M_HV_Cut \t:");
		Debug_PutInt(Y_D_M_HV_Cut);
		
		Debug_PutString("\n\rY_D_LV_Cut \t:");
		Debug_PutInt(Y_D_LV_Cut);	
		Debug_PutString("    Y_D_M_LV_Cut \t:");
		Debug_PutInt(Y_D_M_LV_Cut);
		Debug_PutString("\n\r");		
		
		Debug_PutString("\n\rB_D_ExpV \t:");
		Debug_PutInt(B_D_ExpV);	
		Debug_PutString("    B_D_M_ExpV \t:");
		Debug_PutInt(B_D_M_ExpV);
		
		Debug_PutString("\n\rB_D_Max_VA \t:");
		Debug_PutInt(B_D_Max_VA);	
		Debug_PutString("    B_VA_Max \t:");
		Debug_PutInt(B_VA_Max);
		
		Debug_PutString("\n\rB_D_HV_Cut \t:");
		Debug_PutInt(B_D_HV_Cut);	
		Debug_PutString("    B_D_M_HV_Cut \t:");
		Debug_PutInt(B_D_M_HV_Cut);
		
		Debug_PutString("\n\rB_D_LV_Cut \t:");
		Debug_PutInt(B_D_LV_Cut);	
		Debug_PutString("    B_D_M_LV_Cut \t:");
		Debug_PutInt(B_D_M_LV_Cut);
		Debug_PutString("\n\r");		
	}
